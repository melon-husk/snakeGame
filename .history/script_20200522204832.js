...
const enemies=3;
function drawSnakeMovement(snakeArr){
    let snakeTail = snakeArr[snakeArr.length-1];
    let snakeHead = snakeArr[0];
    // Remove tail
    ctx.fillStyle = "blue";
    ctx.fillRect(snakeTail.x, snakeTail.y, elementWidth, elementHeight);
    // Draw new snake head
    ctx.fillStyle = "green";
    ctx.fillRect(snakeHead.x,snakeHead.y,elementWidth,elementHeight);
    // Set Border
  //  ctx.strokeStyle = "red";
 //   ctx.strokeRect(snakeHead.x,snakeHead.y,elementWidth,elementHeight);
}
...
function initialCanvasDraw(ctx){
    ctx.fillStyle = "blue";
    ...
    //tx.drawImage(enemyImg, enemy.x, enemy.y,elementWidth,elementHeight);  
    
}

function redraw(recursive=false,isFriend=null){
    // old head position
...
initialCanvasDraw(ctx);
//let game = setInterval(redraw,100);
